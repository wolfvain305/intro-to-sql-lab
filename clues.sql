...
WITH least_populated_country AS (
    SELECT code, name
    FROM countries
...
official_language AS (
    SELECT language
    FROM countrylanguages
    WHERE countrycode = (SELECT code FROM least_populated_country)
    AND isofficial = TRUE
)
...
SELECT name
FROM countries
WHERE code != (SELECT code FROM least_populated_country) 
AND code IN (
    SELECT countrycode 
    FROM countrylanguages 
    WHERE language = (SELECT language FROM official_language)
    GROUP BY countrycode
    HAVING COUNT(language) = 1  -- Ensure it's the only language
);
...
AND name NOT LIKE '<OriginalCityEnding>'
AND countrycode IN (
    SELECT code 
    FROM countries 
